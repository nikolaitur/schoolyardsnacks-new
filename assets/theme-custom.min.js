  ...
  (function ($) {
    $(function () {
      var mmStatus = "closed";
      function closeMM() {
        (mmStatus = "closed"), $("#mobile-navigation-toggle").removeClass("opened"), $("body").removeClass("nav-opened");
      }
      function openMM() {
        (mmStatus = "opened"),
          (_body = $("body")),
          ...
          _body.hasClass("mini-cart-open") && $(".minicart-toggle-btn").trigger("click");
      }
      function getCookie(name) {
        return (document.cookie.match("(^|; )" + name + "=([^;]*)") || 0)[2];
      }
      function findCurrentVariant(variantId) {
        return meta.product.variants.filter(function (item) {
          return item.id.toString() === variantId;
        })[0];
      }
      function shouldShowUpgradeModal(variantId) {
        var currentVariant;
        console.log("=====================");
        console.log(findCurrentVariant(variantId).name);
        return findCurrentVariant(variantId).name.split(" / ").includes("12 Bags") && $(".rc_block__type__onetime").hasClass("rc_block__type--active");
      }
      function shouldAddFreeGift() {
        if ("undefined" == typeof promoProduct) return !1;
        var free_gift_item = CartJS.cart.items.filter(function (item) {
          return item.sku === promoProduct.variants[0].sku;
        });
        return $(".product-option-row--free-gift input").is(":checked") && 0 === free_gift_item.length;
      }
      function shouldAddThresholdGift(variantId) {
        var thresholdGiftCookieName;
        if ("undefined" == typeof promoEnabled || "undefined" == typeof promoAmount || !getCookie("sys_recharge_freegift_gauge")) return !1;
        var product = meta.product.variants.filter(function (item) {
          return item.id.toString() === variantId;
        })[0];
        return promoEnabled && CartJS.cart.total_price + product.price >= promoAmount && "onetime" == $(".rc_radio:checked").val();
      }
      function addThresholdGift() {
        if ("undefined" != typeof promoProduct) {
          var cartDoesNotHaveUpsellGift =
            0 ==
            CartJS.cart.items.filter(function (item) {
              return item.id == promoProduct.variants[0].id;
            }).length,
            cartHasNoSubscriptions =
              0 ==
              CartJS.cart.items.filter(function (item) {
                return item.properties.hasOwnProperty("subscription_id");
              }).length;
          cartDoesNotHaveUpsellGift &&
            cartHasNoSubscriptions &&
            CartJS.addItem(
              promoProduct.variants[0].id,
              1,
              {},
              {
      ...
      function showUpgradeModal(variantId) {
        var modalHTML =
          '<div id="12bag-upgrade-modal" style="text-align: center !important;" class="modal" data-variant-id="' +
          variantId +
          '"><h1>Upgrade to 24 Bags and Save 20%</h1><p>82% of customers have taken advantage of this deal.</p><a href="#" class="btn btn--block" id="modal-upgrade-button">Upgrade!</a><a class="btn btn--block" href="#" id="modal-close-button" rel="modal:close">No Thanks</a><a class="" href="#" id="modal-close-button-x" rel="modal:close">×</a></div>';
        0 === $("body #12bag-upgrade-modal").length ? $("body").append(modalHTML) : $("body #12bag-upgrade-modal").replaceWith(modalHTML),
          $("#12bag-upgrade-modal").on($.modal.BEFORE_OPEN, function (event, modal) {
            $(".modal-wrapper").show(), $("#12bag-upgrade-modal").addClass("grid");
          }),
          $("#12bag-upgrade-modal").modal({ blockerClass: "upgrade-modal modal-wrapper", escapeClose: !1, clickClose: !1, showClose: !1 }),
          $("#12bag-upgrade-modal #modal-upgrade-button").on("click", function () {
            var variantId = $("#12bag-upgrade-modal").data("variant-id"),
              currentVariant,
              currentVariantNameItems = meta.product.variants
                .filter(function (item) {
                  return item.id === variantId;
                })[0]
                .name.split("/");
            currentVariantNameItems.pop();
            var upgradedVariant = meta.product.variants.filter(function (item) {
              return item.name.includes(currentVariantNameItems.join("/").replace("12 Bags", "24 Bags"));
            })[0];
      ...
      $("#mobile-navigation-toggle, .mobile-navigation-close").on("click", function () {
        return "closed" == mmStatus ? openMM() : "opened" == mmStatus && closeMM(), !1;
      }),
        $(".mobile-navigation-item__toggle").on("click", function (e) {
          e.preventDefault(), $(this).closest("li").toggleClass("open");
        }),
        ...
        $(document).on("touchend", ".minicart.minicart--visible", function (e) {
          (touchendX = event.changedTouches[0].screenX), touchendX - touchstartX > 100 && $(".minicart__header span").trigger("click");
        }),
        $(document).on("click", "#add-to-cart", function () {
          var urlParams,
            variantId = new URLSearchParams(window.location.search).get("variant");
          console.log("Add to cart upgrade modal");
//           console.log(shouldShowUpgradeModal(variantId));
          
          // functoin for new style product page
          if ((window.location.href.indexOf("products/keto-puffs") > -1 && window.location.href.indexOf("?view=newstyle") > -1) || (window.location.href.indexOf("products/cereal") > -1 && window.location.href.indexOf("?view=newstyle") > -1)) {
            var total_qty = 0;
            jQuery.each($(".flavor-qty-picker"), function (i, item) {
              total_qty += parseInt(item.value);
            });
//             if($(".rc_block.rc_block__type__onetime").hasClass("rc_block__type--active") && total_qty == 1) {
...
//             showUpgradeModal(variantId);
//           }
          // shouldShowUpgradeModal(variantId) && showUpgradeModal(variantId),
          //   !shouldAddFreeGift() && shouldAddThresholdGift(variantId) && (console.log("threshold gift"), addThresholdGift()),
          //   shouldAddFreeGift() &&
        ...
        $("#forgot-password").on("click", function () {
          $("#recover-password-form").show(), $("#customer-login-form").hide();
        }),
        $("#forgot-password-cancel").on("click", function () {
          $("#recover-password-form").hide(), $("#customer-login-form").show();
        });
    });
  })(jQuery);
...
if (window.location.href.includes("ut_content")) {
  var str = window.location.href;
  var campaignValue = str.split('ut_content=').pop();
...
if (localStorage.getItem("utmDiscountCode")) {
  ...
  // $(".header-promo p").html(`Order in the next <span id="timer"></span> 👉 use code <strong class="discountCode">${localStorage.getItem("utmDiscountCode")}</strong> for 10% off. <br /> Free Shipping + 30 Day Money Back Guarantee.`);
...
for (let i = 1; i <= 4; i++) {
  ...
  if (localStorage.getItem("updateCart" + i)) {
    ...
    if (i === 1) {
      ...
      updateCartTitle("Today’s ‘Flash Sale’ Awaits You On The Next Page <span id='cartTimer'></span>");
    } else if (i === 3) {
      updateCartTitle("‘Flash Sale’ Unlocked On Next Page <span id='cartTimer'></span>");
    } else if (i === 4) {
      updateCartTitle("Claim Your ‘Flash Sale’ Code On Next Page <span id='cartTimer'></span>");
    }
    updateCart('cartTime');
  }
}
